# Modification Copyright (c) Databricks Inc.
# Distributed under the terms of the DB License (see https://databricks.com/db-license-source
# for more information).

# Large portions of the code in this file are inlined from https://github.com/quantopian/qgrid/blob/master/qgrid/grid.py
# licensed under the Apache License 2.0 license.

# Modifications:
# - support cythonized function type.
# - renamed Qgrid into BamGrid
...
# # not needed any more because we currently use our own json converter
# import pandas.io.json as pd_json


FUNCTION_TYPE = FunctionType if FunctionType == CyFunctionType else CyFunctionType

# Keep this in sync with /js/src/qgrid.widget.js
PREVIEW_ROW_COUNT = 11
PREVIEW_ROW_BUFFER = 5

# Why 200? See the reasoning: https://databricks.atlassian.net/browse/PROD-26600?focusedCommentId=1503070
# TLDR: The largest number of characters possible that makes sure we don't exceed the 1MB WS limit if
# all rows in a df contain long strings
STRING_LENGTH_LIMIT = 200


def maybe_truncate(string: str, limit: int = STRING_LENGTH_LIMIT):
...
def create_schema(df):
    ...
    index = [{"name": "_bamboolib_index_column", "type": "int64"}]
    columns = [{"name": column, "type": str(df[column].dtype)} for column in df.columns]
    schema["fields"] = index + columns

    schema["primaryKey"] = ["_bamboolib_index_column"]
    schema["pandas_version"] = pd.__version__
    return schema


def create_grid_data_for_normalized_df(df):
    rows_list = []
    for row_index, row in df.iterrows():
        row_dict = {}
        row_dict["_bamboolib_index_column"] = str(row_index)
        # NOTE(PROD-26600): This serialization technique leads to column name replication,
        # i.e. each column name appears PREVIEW_ROW_COUNT + 2*PREVIEW_ROW_BUFFER (21) times in
        # the comm_msg.
        for column, value in row.to_dict().items():
            row_dict[column] = maybe_truncate(str(value))
        rows_list.append(row_dict)
    return rows_list


def create_df_json(df):
    """Create a JSON string from a dataframe."""
    df_json = {}
    df_json["schema"] = create_schema(df)
    df_json["data"] = create_grid_data_for_normalized_df(df)
...
def set_defaults(
    ...
    same ones that you can pass into ``BamGridWidget`` constructor, with the
    ...
    See the documentation for ``BamGridWidget`` for more information.

    Notes:
    This function will be useful to you if you find yourself
    setting the same options every time you create a BamGridWidget. Calling
...
def on(names, handler):
    ...
    :param handler: callable
        ...
        ``event`` is a dictionary and ``qgrid_widget`` is the BamGridWidget
        instance that fired the event. The ``event`` dictionary at least
        holds a ``name`` key which specifies the name of the event that
        occurred.

    Notes:
    There is also an ``on`` method on each individual BamGridWidget instance,
    ...
    module-level and not on individual BamGridWidget instances.
    ...
    BamGridWidget.on :
        ...
        individual BamGridWidget instance rather than on all instances.  See
...
def off(names, handler):
    ...
    :param names: list, str, All (default: All)
...
def show_grid(
    ...
    bamboo_callback_handler=None,
):
    """
    Renders a DataFrame or Series as an interactive qgrid, represented by
    an instance of the ``BamGridWidget`` class.  The ``BamGridWidget`` instance
    ...
    call to the BamGridWidget constructor, and the rest of the parameters
    are passed through as is.

    If the ``data_frame`` argument is a Series, it will be converted to a
    DataFrame before being passed in to the BamGridWidget constructor as the
    ``df`` kwarg.
    ...
    :param row_edit_callback: callable
    ...
    for BamGrid and therefore are not documented in the SlickGrid documentation.
    The following bullet points describe these options.

    * **maxVisibleRows** The maximum number of rows that BamGrid will show.
    * **minVisibleRows** The minimum number of rows that BamGrid will show
    * **sortable** Whether the BamGrid instance will allow the user to sort
      columns by clicking the column headers. When this is set to ``False``,
      nothing will happen when users click the column headers.
    * **filterable** Whether the BamGrid instance will allow the user to filter
    ...
    * **highlightSelectedRow** If you set this to False, the light blue
    ...
    The ``editable`` option was added specifically for BamGrid and therefore is
    ...
    set_defaults : Permanently set global defaults for the parameters
                   ...
                   to set for all BamGridWidet instances.
    ...
    BamGridWidget : The widget class that is instantiated and returned by this
                  method.
    ...
    return BamGridWidget(
        df=data_frame,
        ## other kwargs are currently disabled
        ## if we want them to work, we need to set them in the constructor of BamGridWidget
        # precision=precision,
        # grid_options=grid_options,
        # column_options=column_options,
        ...
        # show_toolbar=show_toolbar,
        # bamboo_callback_handler=bamboo_callback_handler,
    )
...
@widgets.register()
class BamGridWidget(widgets.DOMWidget):
    """
    ...
    See Also:
    show_grid : The method that should be used to construct BamGridWidget
    ...
    # _df = Instance(pd.DataFrame)  # if it is set as a trait it takes too long to load
    ...
    # _ignore_df_changed = Bool(False)  # if it is set as a trait it takes too long to load
    # _unfiltered_df = Instance(pd.DataFrame)  # if it is set as a trait it takes too long to load
    ...
    _viewport_range = Tuple(
        Integer(), Integer(), default_value=(0, PREVIEW_ROW_COUNT)
    ).tag(sync=True)
    _df_range = Tuple(Integer(), Integer(), default_value=(0, PREVIEW_ROW_COUNT)).tag(
        sync=True
    )
    ...
    # df = Instance(pd.DataFrame)  # if it is set as a trait it takes too long to load
    precision = Integer(6).tag(sync=True)
    grid_options = Dict().tag(sync=True)
    column_options = Dict({})
    column_definitions = Dict({})
    show_toolbar = Bool(False).tag(sync=True)
    row_edit_callback = Instance(FUNCTION_TYPE, allow_none=True).tag(sync=False)
    bamboo_callback_handler = Instance(FUNCTION_TYPE, allow_none=True).tag(sync=False)
    id = Unicode().tag(sync=True)

    def __init__(self, df=None, *args, **kwargs):
        self.df = df
        self.df_column_indices = None

        self.id = str(uuid4())
        self._initialized = False
        ## the next call loses a lot of time - most likely because ipywidgets does some serialization on the df
    ...
    def set_bamboo_callback_handler(self, bamboo_callback_handler):
        self.bamboo_callback_handler = bamboo_callback_handler

    def refresh_grid(self):
        self.send({"type": "bam_refresh_grid"})
    ...
    def on(self, names, handler):
        ...
        Notes:
        Here's the list of events that you can listen to on BamGridWidget
        ...
        * **json_updated** A user action causes BamGridWidget to send rows of
        ...
        * **viewport_changed** The user scrolled the new rows into view in
        ...
        on :
            ...
            events on all instances rather than on an individual BamGridWidget
            instance.
        BamGridWidget.off:
    ...
    def _update_df(self):
        self._reset_df_and_viewport_range()
        ...
        self._unfiltered_df = self._df

        self._update_table(update_columns=True, fire_data_change_event=False)
        self._ignore_df_changed = False

    def _reset_df_and_viewport_range(self):
        self._viewport_range = (0, PREVIEW_ROW_COUNT)
        self._df_range = (0, PREVIEW_ROW_COUNT)

    def _rebuild_widget(self):
        self._update_df()
        self.send({"type": "draw_table"})

    def update_df(self, df, df_column_indices):
        self.df = df
        self.df_column_indices = df_column_indices
        self._rebuild_widget()

    @observe("precision")
    def _on_precision_changed(self, change):
        if not self._initialized:
            return
        self._rebuild_widget()

    @observe("grid_options")
    def _on_grid_options_changed(self, change):
        if not self._initialized:
            return
        self._rebuild_widget()

    @observe("show_toolbar")
    def _on_show_toolbar_changed(self, change):
        if not self._initialized:
            return
    ...
    def _update_table(
        ...
        from_index = max(self._viewport_range[0] - PREVIEW_ROW_BUFFER, 0)
        to_index = max(
            self._viewport_range[0] + PREVIEW_ROW_COUNT + PREVIEW_ROW_BUFFER, 0
        )
        ...
        if self.df_column_indices is not None:
            df = df[self.df_column_indices]
        # perform the copy after we did get the subset
        # this protects the performance on big data
        # if we don't do the copy, then the original df will be changed inplace in object columns that do not hold strings
        df = df.copy()

        # + 1 to fix chrome qgrid bug
        self._row_count = len(self._df.index) + 1
        ...
        if update_columns:
            ...
            if type(df.index) == pd.MultiIndex:
                ...
                if df.index.name:
                    ...
                    col_name = "_bamboolib_index_column"
                    self._primary_key_display[col_name] = ""
    ...
    def _handle_change_filter(self, content):
        ...
        self._columns = columns
        self._print_column_filters(columns)
    ...
    def _create_column_filter_string(self, column_name, filter_info):
        if filter_info["type"] == "slider":
            if filter_info["min"] is not None:
                return f"(df[{string_to_code(column_name)}] >= {filter_info['min']})"
            if filter_info["max"] is not None:
                return f"(df[{string_to_code(column_name)}] <= {filter_info['min']})"

    def _print_column_filters(self, columns):
        try:
            from IPython.core.getipython import get_ipython
        except ImportError:
            raise ImportError("This feature requires IPython 1.0+")
        ...
        for column_name, column in columns.items():
            if "filter_info" in column:
                column_filters.append(
                    self._create_column_filter_string(
                        column_name, column["filter_info"]
                    )
                )

        all_filters_string = " & ".join(column_filters)
        result = f"df[{all_filters_string}]"

        display(result)
        get_ipython().set_next_input(result)
    ...
    def _handle_qgrid_msg_helper(self, content):
        """Handle incoming messages from the BamGridView"""
        if "type" not in content:
            return

        if "bam" in content["type"]:
            self.bamboo_callback_handler(content)
            return
        ...
        if content["type"] == "edit_cell":
            ...
            # Note(PROD-27100): BamGridView.slick_grid.onViewportChanged always expects a reply
            # from the backend when it has sent a change_viewport message.
            # On databricks, it can happen that change_viewport is send while old_viewport_range == self._viewport_range
            # We thus always need to reply, independent of the _viewport_range
            # See https://databricks.atlassian.net/browse/PROD-27100 for more details.
            ...
            from_index = max(content["top"] - PREVIEW_ROW_BUFFER, 0)
            to_index = max(content["top"] + PREVIEW_ROW_COUNT + PREVIEW_ROW_BUFFER, 0)
    ...
    def get_changed_df(self):
        ...
        instance of BamGridWidget which reflects the current state of the UI.
    ...
    def remove_rows(self, rows=None):
        ...
        :param rows: list (default: None)
            ...
            the MultiIndex.
        """
        indices = self._remove_rows(rows=rows)
        self._notify_listeners(
            {"name": "row_removed", "indices": indices, "source": "api"}
        )
        return indices
    ...
    def _change_selection(self, rows, source):
